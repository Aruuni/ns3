...
EXPERIMENT = "fairness_with_cubic_rtt"
...
def run_simulation(params):
    ...
    command = "./ns3 run --no-build \"scratch/SimulatorScript.cc --stopTime={} --flowStartOffset={} --appendFlow={} --appendFlow2={} --queueBDP={} --botLinkDelay={} --botLinkDataRate={} --path={}/bw{}/delay{}/qmult{}/flows2/{}/run{} --seed={}\"".format(DURATION, SECONDFLOWSTART, 'TcpCubic', protocol, mult, delay, bw, EXPERIMENT, bw, delay, mult, protocol, run, run)
...
for mult in QMULTS:
     ...
     for bw in BWS:
        for delay in DELAYS:
           ...
           for run in RUNS:
              ...
              if os.path.exists(PATH + '/TcpCubic0-goodput.csv') and os.path.exists(PATH + '/'+protocol+'1-goodput.csv'):
                receiver1_total = pd.read_csv(PATH + '/TcpCubic0-goodput.csv').reset_index(drop=True)
   ...
   markers, caps, bars = ax.errorbar(TcpCubic_data.index*2, TcpCubic_data['goodput_ratio_total_mean'], yerr=TcpCubic_data['goodput_ratio_total_std'],marker='x',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK, label='cubic')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]
   markers, caps, bars = ax.errorbar(TcpBbr3_data.index*2, TcpBbr3_data['goodput_ratio_total_mean'], yerr=TcpBbr3_data['goodput_ratio_total_std'],marker='^',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK,label='bbr')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]
   markers, caps, bars = ax.errorbar(TcpBbr_data.index*2,TcpBbr_data['goodput_ratio_total_mean'], yerr=TcpBbr_data['goodput_ratio_total_std'],marker='+',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK,label='bbr1')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]
