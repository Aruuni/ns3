...
EXPERIMENT = "fairness_with_itself_bw"
...
for mult in QMULTS:
   ...
   summary_data = pd.DataFrame(data,
    ...
    # PROTOCOLS = ['TcpCubic', 'TcpBbr', 'TcpBbr3']
   TcpCubic_data = summary_data[summary_data['protocol'] == 'TcpCubic'].set_index('bandwidth')
   TcpBbr_data = summary_data[summary_data['protocol'] == 'TcpBbr'].set_index('bandwidth')
   TcpBbr3_data = summary_data[summary_data['protocol'] == 'TcpBbr3'].set_index('bandwidth')
   ...
   markers, caps, bars = ax.errorbar(TcpCubic_data.index, TcpCubic_data['goodput_ratio_total_mean'], yerr=TcpCubic_data['goodput_ratio_total_std'],marker='x',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK, label='cubic')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]
   markers, caps, bars = ax.errorbar(TcpBbr3_data.index,TcpBbr3_data['goodput_ratio_total_mean'], yerr=TcpBbr3_data['goodput_ratio_total_std'],marker='^',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK,label='bbr')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]
   markers, caps, bars = ax.errorbar(TcpBbr_data.index,TcpBbr_data['goodput_ratio_total_mean'], yerr=TcpBbr_data['goodput_ratio_total_std'],marker='+',linewidth=LINEWIDTH, elinewidth=ELINEWIDTH, capsize=CAPSIZE, capthick=CAPTHICK,label='bbr1')
   [bar.set_alpha(0.5) for bar in bars]
   [cap.set_alpha(0.5) for cap in caps]

   ax.set(yscale='linear',xlabel='Bandwidth (Mbps)', ylabel='Goodput Ratio', ylim=[-0.1,1.1])
